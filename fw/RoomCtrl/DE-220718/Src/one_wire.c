    ...
    else if(IsONEWIRE_ThermostatConnected())    // use onewire bus as usart half duplex
	{
        ...
        switch(ow_cmd)
        {
            ...
            case ONEWIRE_GET_STATE:
            {
                ...
                /** ==========================================================================*/
                if((ow_buff[2] & (1U << 0)) != 0U) LogEvent.log_event = FANCOIL_RPM_SENSOR_ERROR;
                if((ow_buff[2] & (1U << 1)) != 0U) LogEvent.log_event = FANCOIL_NTC_SENSOR_ERROR;
                if((ow_buff[2] & (1U << 2)) != 0U) LogEvent.log_event = FANCOIL_LO_TEMP_ERROR;
                if((ow_buff[2] & (1U << 3)) != 0U) LogEvent.log_event = FANCOIL_HI_TEMP_ERROR;
                if((ow_buff[2] & (1U << 4)) != 0U) LogEvent.log_event = FANCOIL_FREEZING_PROTECTION;
                if((ow_buff[2] & (1U << 5)) != 0U) LogEvent.log_event = AMBIENT_NTC_SENSOR_ERROR;

                if((ow_buff[2] & 0x3FU) != 0U)
                {
                    LogEvent.log_card_id[0] = (ow_pcnt + 1U);
                    LOGGER_Write();
                }
                /** ==========================================================================*/
...
static void ONEWIRE_SetUsart(uint32_t setup)
{
    ...
    if (HAL_UART_DeInit(&huart2) != HAL_OK) Error_Handler(ONEWIRE_FUNC, USART_DRIVER);
    if (HAL_UART_Init(&huart2) != HAL_OK) Error_Handler(ONEWIRE_FUNC, USART_DRIVER);
}
...
static uint8_t ONEWIRE_Reset(void)
{
	...
	onew_buff[0] = 0xF0U;
    onew_buff[1] = 0U;
	if (HAL_UART_Transmit(&huart2, onew_buff, 1U, ONEWIRE_TIMEOUT) != HAL_OK) Error_Handler(ONEWIRE_FUNC, USART_DRIVER);
	if (HAL_UART_Receive(&huart2, &onew_buff[1], 1U, ONEWIRE_TIMEOUT) != HAL_OK) Error_Handler(ONEWIRE_FUNC, USART_DRIVER);
