...
uint8_t SPI_FLASH_ReadByte(uint32_t address)
{
    ...
    spi_buff[3] = address; 
    spi_buff[4] = 0U;
    if (HAL_SPI_Transmit(&hspi2, spi_buff, 4U, SPI_FLASH_TIMEOUT) != HAL_OK) Error_Handler(EEPROM_FUNC, SPI_DRIVER);
    if (HAL_SPI_Receive(&hspi2, &spi_buff[4], 1U, SPI_FLASH_TIMEOUT) != HAL_OK) Error_Handler(EEPROM_FUNC, SPI_DRIVER);
...
uint16_t SPI_FLASH_ReadInt(uint32_t address)
{
    ...
    spi_buff[5] = 0U;
    if (HAL_SPI_Transmit(&hspi2, spi_buff, 4U, SPI_FLASH_TIMEOUT) != HAL_OK) Error_Handler(EEPROM_FUNC, SPI_DRIVER);
    if (HAL_SPI_Receive(&hspi2, &spi_buff[4], 2U, SPI_FLASH_TIMEOUT) != HAL_OK) Error_Handler(EEPROM_FUNC, SPI_DRIVER);
    HAL_GPIO_WritePin(FLASH_CS_Port, FLASH_CS_Pin, GPIO_PIN_SET);
...
void SPI_FLASH_ReadPage(uint32_t address, uint8_t *data, uint16_t size)
{
    ...
    spi_buff[3] = address;
    if (HAL_SPI_Transmit(&hspi2, spi_buff, 4U, SPI_FLASH_TIMEOUT) != HAL_OK) Error_Handler(EEPROM_FUNC, SPI_DRIVER);
    if (HAL_SPI_Receive(&hspi2, data, size, SPI_FLASH_TIMEOUT) != HAL_OK) Error_Handler(EEPROM_FUNC, SPI_DRIVER);
    HAL_GPIO_WritePin(FLASH_CS_Port, FLASH_CS_Pin, GPIO_PIN_SET);
}
...
void SPI_FLASH_WritePage(uint32_t address, uint8_t *data, uint16_t size)
{
    ...
    spi_buff[3] = address;
    if (HAL_SPI_Transmit(&hspi2, spi_buff, 4U, SPI_FLASH_TIMEOUT) != HAL_OK) Error_Handler(EEPROM_FUNC, SPI_DRIVER);
    if (HAL_SPI_Transmit(&hspi2, data, size, SPI_FLASH_TIMEOUT) != HAL_OK) Error_Handler(EEPROM_FUNC, SPI_DRIVER);
    HAL_GPIO_WritePin(FLASH_CS_Port, FLASH_CS_Pin, GPIO_PIN_SET);
}
...
uint8_t SPI_FLASH_ReleasePowerDown(void)
{
    ...
    if (HAL_SPI_Transmit(&hspi2, spi_buff, 1U, SPI_FLASH_TIMEOUT) != HAL_OK) Error_Handler(EEPROM_FUNC, SPI_DRIVER);
    HAL_GPIO_WritePin(FLASH_CS_Port, FLASH_CS_Pin, GPIO_PIN_SET);
    HAL_Delay(1);
...
void EEPROM_Save(uint16_t ee_address, uint8_t* value, uint8_t size)
{
    ...
    if (HAL_I2C_Master_Transmit(&hi2c1, I2C_EE_WRITE, ee_buff, t, I2C_EE_TIMEOUT) != HAL_OK) Error_Handler(EEPROM_FUNC, I2C_DRIVER);
    if (HAL_I2C_IsDeviceReady(&hi2c1, I2C_EE_READ, I2C_EE_TRIALS, I2C_EE_WRITE_DELAY) != HAL_OK) Error_Handler(EEPROM_FUNC, I2C_DRIVER);
}
...
static uint8_t SPI_FLASH_ReadStatusRegister(void)
{
    ...
    spi_buff[1] = 0U;
    if (HAL_SPI_Transmit(&hspi2, spi_buff, 1U, SPI_FLASH_TIMEOUT) != HAL_OK) Error_Handler(EEPROM_FUNC, SPI_DRIVER);
	if (HAL_SPI_Receive(&hspi2, &spi_buff[1], 1U, SPI_FLASH_TIMEOUT) != HAL_OK) Error_Handler(EEPROM_FUNC, SPI_DRIVER);
    HAL_GPIO_WritePin(FLASH_CS_Port, FLASH_CS_Pin, GPIO_PIN_SET);
    return (spi_buff[1]);
}
...
static uint8_t SPI_FLASH_ReadDeviceInfo(void)
{
    ...
    spi_buff[3] = 0U;
	if (HAL_SPI_Transmit(&hspi2, spi_buff, 1U, SPI_FLASH_TIMEOUT) != HAL_OK) Error_Handler(EEPROM_FUNC, SPI_DRIVER);
	if (HAL_SPI_Receive(&hspi2, &spi_buff[1], 3U, SPI_FLASH_TIMEOUT) != HAL_OK) Error_Handler(EEPROM_FUNC, SPI_DRIVER);
	HAL_GPIO_WritePin(FLASH_CS_Port, FLASH_CS_Pin, GPIO_PIN_SET);
