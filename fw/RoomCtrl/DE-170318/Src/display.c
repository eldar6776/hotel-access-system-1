...
/* Private Constante ---------------------------------------------------------*/
const char *label [3][1] = {"ERROR", "LuxRT.x ", "DS18B.x "};
const char *item  [8][1] = {"RESTART CONTROLER         ",   "PREVIEW IMAGE             ",   "UPDATE IMAGE              ",   "UPDATE FIRMWARE           ", 
                            "SCAN ONEWIRE BUS:         ",   "SET RS485 ADDRESS:        ",   "SET SYSTEM ID:            ",   "SET TIME:                 "};
const char *taster[3][1] = {"ENTER                 EXIT",   "NEXT >>             SELECT",   "SET ++            DESELECT"};
...
{
    uint8_t disp_buff[DISPLAY_MESSAGE_SIZE], read_byte;
...
//    static uint32_t jrn_char = 0U;
    ...
    /*************************************************************************/
    read_byte = ILI9341_RD_DISP_POWER_MODE;
    ...
    if (HAL_SPI_Transmit(&hspi2, &read_byte, 1U, SPI_FLASH_TIMEOUT) != HAL_OK) Error_Handler();
    if (HAL_SPI_Receive(&hspi2, &read_byte, 1U, SPI_FLASH_TIMEOUT) != HAL_OK) Error_Handler();
    HAL_GPIO_WritePin(DISPLAY_CS_Port, DISPLAY_CS_Pin, GPIO_PIN_SET);
    HAL_GPIO_WritePin(DISPLAY_DC_Port, DISPLAY_DC_Pin, GPIO_PIN_SET);
    
    if((read_byte & (1U << 7)) == 0U)
    {
#ifndef USE_DEBUGGER  
    ...
    /*************************************************************************/
    else if(IsDISPLAY_PreviewImageActiv())
    {
    ...
    /*************************************************************************/
    /**   C H E C K  F O R  V A L I D  C O N D I T I O N S  T O  P A S S    **/
    /*************************************************************************/ 
    if(((SignalBuzzer != BUZZ_OFF) || IsBuzzerActiv() || IsRS485_UpdateActiv()) && !IsDISPLAY_MenuActiv()) return;
    /*************************************************************************/
    /**         E V E N T   =   U S E R     C A R D     V A L I D           **/
    /*************************************************************************/   
    if(IsDISPLAY_CardValidImageActiv()) 
    {
        ...
        if(IsDISPLAY_UserCardInfoTextEnabled())
        {
            ...
            if(sCard.user_group == CARD_USER_GROUP_GUEST) DISPLAY_CardUserName();
            else DISPLAY_CardValidDate();
        }
        /*************************************************************************/
...
static void DISPLAY_PrintMenuValue(sMenuTypeDef* menu)
{
    ...
    DISPLAY_PrintString(0, Line0_M, "    SYSTEM  SETTINGS      ");
    ...
    for(c = 0U; c < 8U; c ++)
    {
        ...
        DISPLAY_PrintString(0, Line2_M + (c * Line1_M), item[c][0]);

        if(menu->item_type[c] == VALUE_IN_BUFFER)
        {
            if(c == 7U)
            {
                Hex2Str(&menu->item_buff[7][0], 1U, &rtc[0]);     // BCD hours value 0x01~0x23
                rtc[2] = ':';
                Hex2Str(&menu->item_buff[7][1], 1U, &rtc[3]);     // BCD  minute value 0x00~0x59
                rtc[5] = ' ';
                Hex2Str(&menu->item_buff[7][2], 1U, &rtc[6]);     // BCD  date value 0x01~0x31
                rtc[8] = '.';
                Hex2Str(&menu->item_buff[7][3], 1U, &rtc[9]);     // BCD  month value 0x01~0x12
                rtc[11] = '.';
                rtc[12] = '2';
                rtc[13] = '0';
                Hex2Str(&menu->item_buff[7][4], 1U, &rtc[14]);   // BCD  year value 0x18~0x99
                rtc[16] = 0U;
        ...
        else if(menu->item_type[c] == LABEL_ITEM)
        {
            back_color = LGRAY;
            pixel_color = BLACK;
            DISPLAY_PrintString(210, Line6_M, label[menu->item_label[4]][0]);
            DISPLAY_PrintCharacter(300, Line6_M, onewire_device_connected + '0');            
        }
    }        
}
...
static void DISPLAY_MenuService(sMenuTypeDef* menu)
{
    ...
    /*************************************************************************/    
    if(menu->select == 2U)  // screen item value edit mode selected
    {
        ...
        {
            ONEWIRE_ScanBus();
            if(IsONEWIRE_DalasSensorConnected()) menu->item_label[4] = 2U;
            else if(IsONEWIRE_ThermostatConnected()) 
            {
                menu->item_label[4] = 1U;
                EEPROM_Save(EE_ONEWIRE_ADDRESS_1, thermostat_addresse, 9U);
            }
            else menu->item_label[4] = 0U;
...
static void DISPLAY_DateTime(void)
{
    ...
    static uint32_t temp_sec = 0U, temp_day = 0U;
    
    HAL_RTC_GetTime(&hrtc, &time_d, RTC_FORMAT_BCD);
    if(temp_sec == time_d.Seconds) return;
    temp_sec = time_d.Seconds;
    /***********************************************************************
    **		    B A C K U P     R T C       D A T E	    D A I L Y
    ***********************************************************************/
...
static void DISPLAY_CardInfoText(uint8_t user)
{
    ...
    {
        if(rc522_rx_buff[0] & (1U << 7))                                                              
        {
            rc522_rx_buff[0] &= (~(1U << 7));   DISPLAY_PrintString(130U, Line6_B, " Mr.");     // guest card user is mister
        }
...
static void DISPLAY_Image(uint8_t selected)
{
    ...
    while(image_size)
	{
		...
		HAL_GPIO_WritePin(DISPLAY_DC_Port, DISPLAY_DC_Pin, GPIO_PIN_SET);
		HAL_GPIO_WritePin(DISPLAY_CS_Port, DISPLAY_CS_Pin, GPIO_PIN_RESET);
		if (HAL_SPI_Transmit(&hspi2, dsp_buff, char_cnt, LCD_DISPLAY_TIMEOUT) != HAL_OK) Error_Handler();
		HAL_GPIO_WritePin(DISPLAY_CS_Port, DISPLAY_CS_Pin, GPIO_PIN_SET);
