		...
		{	
			if(sCard.user_group == CARD_USER_GROUP_GUEST)
			{
				...
				else
				{
					...
					if (HAL_I2C_Master_Transmit(&hi2c1, I2C_EE_WRITE, ee_buff, 7U, I2C_EE_TIMEOUT) != HAL_OK) Error_Handler();
			...
			else if(handmaid_card_cycles == 0U)
			{
                if(IsDISPLAY_UserCardInfoTextEnabled())
                {
                    ...
                    */
                    ...
                    while((i < 16U) && (sector_0.block_2[i] != 0xff))	// copy user data from card to display buffer
                    {	
...
static void RC522_WriteRegister(uint8_t addr, uint8_t val) 
{
	...
	if(HAL_I2C_Master_Transmit(&hi2c1, RC522_WRITE, rc522_tx_buff, 2U, RC522_TIMEOUT) != HAL_OK) Error_Handler();
}
...
{
	rc522_tx_buff[0] = addr;
	if(HAL_I2C_Master_Transmit(&hi2c1, RC522_WRITE, rc522_tx_buff, 1U, RC522_TIMEOUT) != HAL_OK) Error_Handler();
	if(HAL_I2C_Master_Receive(&hi2c1, RC522_READ, rc522_rx_buff, 1U, RC522_TIMEOUT) != HAL_OK) Error_Handler();
...
static RC522_StatusTypeDef RC522_VerifyData(void)
{
	...
	**/	
	ee_buf[0] = (EE_PERMITED_GROUP_ADDRESS >> 8U);
	ee_buf[1] = (EE_PERMITED_GROUP_ADDRESS & 0xFFU);
	if(HAL_I2C_Master_Transmit(&hi2c1, I2C_EE_WRITE, ee_buf, 2U, I2C_EE_TIMEOUT) != HAL_OK) Error_Handler();
	if(HAL_I2C_Master_Receive(&hi2c1, I2C_EE_READ, ee_buf, 16U, I2C_EE_TIMEOUT) != HAL_OK) Error_Handler();
	...
	**/
	...
	if (((sector_2.block_0[0] >> 4U) > 3U) || (((sector_2.block_0[0] >> 4U) ==  0U) && \
		((sector_2.block_0[0] & 0x0FU) == 0U)) || ((sector_2.block_0[0] & 0x0FU) > 9U))
	{
		sCard.expiry_time[0] = EXPIRY_TIME_DATA_INVALID;
	}
	else if (((sector_2.block_0[1] >> 4U) > 1U) || (((sector_2.block_0[1] >> 4U) == 0U) && \
			((sector_2.block_0[1] & 0x0FU) == 0U)) || ((sector_2.block_0[1] & 0x0FU) > 9U))
	{
		sCard.expiry_time[1] = EXPIRY_TIME_DATA_INVALID;
	}
	else if (((sector_2.block_0[2] >> 4U) > 9U) || ((sector_2.block_0[2] & 0x0FU) > 9U))
	{
		sCard.expiry_time[2] = EXPIRY_TIME_DATA_INVALID;
	}
	else if (((sector_2.block_0[3] >> 4U) > 2U) || ((sector_2.block_0[3] & 0x0FU) > 9U))
	{
		sCard.expiry_time[3] = EXPIRY_TIME_DATA_INVALID;
	}
	else if (((sector_2.block_0[4] >> 4U) > 5U) || ((sector_2.block_0[4] & 0x0FU) > 9U))
	{
