...
void Str2Hex(uint8_t *p_str, uint16_t lenght, uint8_t *p_hex)
{
 ...
 while(lenght)
 {
  if((p_str[str_cnt] - 48U) > 9U) p_hex[hex_cnt] = ((((p_str[str_cnt]) >= 'A') && ((p_str[str_cnt]) <= 'F')) ? ((p_str[str_cnt]) - 'A'+10) : ((p_str[str_cnt]) - 'a'+10)) << 4U;
  else p_hex[hex_cnt] = (p_str[str_cnt] - '0') << 4U;
  ++str_cnt;
  if((p_str[str_cnt] - 48U) > 9U) p_hex[hex_cnt] += ((((p_str[str_cnt]) >= 'A') && ((p_str[str_cnt]) <= 'F')) ? ((p_str[str_cnt]) - 'A'+10) : ((p_str[str_cnt]) - 'a'+10));
...
void Hex2Str(uint8_t *p_hex, uint16_t lenght, uint8_t *p_str)
{
 ...
 while(lenght)
 {
  if((p_hex[hex_cnt] >> 4U) > 9U) p_str[str_cnt] = (p_hex[hex_cnt] >> 4U) + 55U;
  else p_str[str_cnt] = (p_hex[hex_cnt] >> 4U) + 48U;
  ++str_cnt;
  if((p_hex[hex_cnt] & 0x0fU) > 9U) p_str[str_cnt] = (p_hex[hex_cnt] & 0x0fU) + 55U;
...
{
 calc = 0U;
 while(calc < size) buffer[calc++] = 0U;
}
...
uint32_t Str2Int(uint8_t *p_inputstr, uint32_t *p_intnum)
{
  ...
  if ((p_inputstr[0] == '0') && ((p_inputstr[1] == 'x') || (p_inputstr[1] == 'X')))
  {
    ...
    while ((i < 11U) && ( p_inputstr[i] != '\0' ) )
    {
      ...
      {
        val = (val << 4U) + ((((p_inputstr[i]) >= '0') && ((p_inputstr[i]) <= '9')) ? ((p_inputstr[i]) - '0') : ((((p_inputstr[i]) >= 'A') && ((p_inputstr[i]) <= 'F')) ? ((p_inputstr[i]) - 'A'+10) : ((p_inputstr[i]) - 'a'+10)));
      }
      else
      {
